...
taskade_reoccurring_leetcode_daily_task_id = os.environ[
    "taskade_reoccurring_leetcode_daily_task_id"
]
...
for event in push_events:
    if (
        ...
        request = urllib.request.Request(
            url_task_complete, headers=headers, method="POST"
        )
        with urllib.request.urlopen(request) as response:
            print(response.code)
        break
...
# for leetcode_daily_event in events_interested_in_leetcode_dailies_only:
#     if (
#         dtelta := (
#             datetime.now()
#             - datetime.strptime(event["created_at"], "%Y-%m-%dT%H:%M:%SZ")
...
#     ).days == 0:
#         print(True)
#         url_task_complete = f"https://www.taskade.com/api/v1/projects/{taskade_reoccurring_project_id}/tasks/{taskade_reoccurring_leetcode_daily_task_id}/complete"
#         headers = {"Authorization": f"Bearer {taskade_api_pat}"}
#         request = urllib.request.Request(
#             url_task_complete, headers=headers, method="POST"
#         )
...
# else:
#     print(False, "no leetcode_dailies push events today")
